...
test('달빛 주방열차 Stage 1 AI 영상이 목업 페이지에서 재생된다', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /두 AI가 첫 주문을/ })).toBeVisible();
  const video = page.locator('#stage-video');
  await expect(video).toBeVisible();
  await expect.poll(() => video.evaluate((element) => element.duration)).toBeGreaterThan(66);
  await expect(video.locator('source')).toHaveAttribute('src', 'assets/moonlight-kitchen-ai-stage1-1280x720.mp4');
  const serveMoment = page.getByRole('button', { name: '01:01 · 1번 역 배식' });
  await expect(serveMoment).toHaveAttribute('data-time', '61');
